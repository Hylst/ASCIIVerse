        ...
        <div className="flex flex-col h-full gap-4 min-h-0">
            ...
            <div className="flex flex-col lg:flex-row gap-4 flex-1 min-h-0">
                ...
                <div className="flex-1 flex flex-col gap-4 min-h-0 min-w-0 order-2 lg:order-1">
                    ...
                    {/* Header / Tabs */}
                    <div className="bg-card rounded-xl border border-border w-full shrink-0">
                    ...
                    {/* Selection Grid */}
                    <div className="flex-1 bg-card rounded-xl border border-border p-4 overflow-y-auto custom-scrollbar">
                ...
                <div className="w-full lg:w-96 flex flex-col gap-4 shrink-0 order-1 lg:order-2">
                    ...
                    <div className="bg-card rounded-xl shadow-lg border border-border overflow-hidden flex flex-col shrink-0">
                        {/* Result Display */}
                        <div className="h-20 bg-muted/30 flex items-center justify-center px-4 relative border-b border-border">
                            <input
                                type="text"
                                value={currentOutput}
                            ...
                            />
                            {manualOverride !== null && (
                                <span className="absolute top-1 right-2 text-[9px] text-muted-foreground uppercase font-bold tracking-wider">Edited</span>
                            )}
                        </div>
                        ...
                        <div className="p-3 bg-card flex flex-col gap-2">
                            <div className="flex gap-2">
                                ...
                                <button onClick={() => randomize()} className="px-4 py-2 rounded-lg bg-accent/10 text-accent font-bold hover:bg-accent hover:text-accent-foreground transition-colors" title="Randomize">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z" /><polyline points="3.27 6.96 12 12.01 20.73 6.96" /><line x1="12" y1="22.08" x2="12" y2="12" /></svg>
                                </button>
                                <button onClick={() => { setManualOverride(null); setBase(0); setEye(0); setMouth(0); setArm(0); setAccessory(0); }} className="px-4 py-2 rounded-lg bg-muted text-muted-foreground font-bold hover:bg-muted/80 transition-colors" title="Reset">
                        ...
                        {/* Compact Moods */}
                        <div className="px-3 pb-3 bg-card flex gap-2 overflow-x-auto custom-scrollbar">
            ...
            <div className="h-40 bg-card rounded-xl border border-border flex flex-col shrink-0 order-3">
                <div className="p-3 border-b border-border flex justify-between items-center bg-muted/30">
                    <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">History</h4>
                    {history.length > 0 && (
                        <button onClick={() => setHistory([])} className="text-[10px] text-destructive hover:text-destructive/80">Clear</button>
                    )}
                </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                            {history.map((item, idx) => (
                                <button
                                    ...
                                    className="text-left p-2 rounded bg-muted/50 hover:bg-muted text-foreground/80 font-mono text-xs border border-transparent hover:border-border transition-all flex justify-between group"
                                >
                                    <span className="truncate">{item}</span>
                                    <span className="opacity-0 group-hover:opacity-100 text-[10px] text-accent self-center">LOAD</span>
                                </button>
                            ))}
                        </div>
                    )}
...
const MoodBtn = ({ label, icon, onClick }: { label: string, icon: string, onClick: () => void }) => (
    <button onClick={onClick} className="flex-none flex items-center gap-1 px-3 py-1 bg-muted border border-border rounded-full text-xs font-medium hover:bg-accent/10 hover:border-accent hover:text-accent transition-colors">
