...
const SmileyLibrary: React.FC = () => {
    ...
    return (
        <div className="flex flex-col lg:flex-row h-full gap-4 min-h-0 bg-background text-foreground relative">

            {/* --- LEFT SIDEBAR (Navigation) --- */}
            <div className="w-full lg:w-48 bg-card rounded-xl border border-border flex flex-col shrink-0 overflow-hidden h-auto lg:h-full">
                ...
                <div className="flex-1 overflow-x-auto lg:overflow-y-auto custom-scrollbar flex lg:flex-col p-2 gap-1">
                    {categories.map(cat => (
                        <button
                            ...
                            className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all text-left whitespace-nowrap ${activeCategory === cat
                ...
                <div className="hidden lg:block p-3 border-t border-border">
                    <div className="text-[10px] font-bold text-muted-foreground uppercase mb-2">Skin Tone</div>
                    <div className="flex justify-between">
                        {(['', '🏻', '🏼', '🏽', '🏾', '🏿'] as SkinTone[]).map(tone => (
                            <button
                                key={tone}
                                onClick={() => setSkinTone(tone)}
                                className={`w-5 h-5 rounded-full border border-border transition-transform hover:scale-110 ${skinTone === tone ? 'ring-2 ring-accent' : ''}`}
            ...
            <div className="flex-1 flex flex-col gap-4 min-h-0">
                ...
                {/* Top Bar */}
                <div className="bg-card p-3 rounded-xl border border-border flex gap-4 items-center shrink-0">
                    <div className="relative flex-1">
                        <input
                            ...
                            className="w-full bg-background border border-border rounded-lg pl-9 pr-4 py-2 text-sm text-foreground focus:border-accent focus:outline-none"
                        />
                ...
                {/* Emoji Grid */}
                <div className="flex-1 bg-background rounded-xl border border-border p-4 overflow-y-auto custom-scrollbar">
                    <div className="grid grid-cols-5 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-8 gap-3">
                        {filteredItems.map(item => (
                            <button
                                ...
                                className={`aspect-square rounded-xl flex items-center justify-center text-3xl transition-all relative group select-none ${selectedEmoji?.id === item.id
                                    ? 'bg-muted ring-2 ring-accent'
                                    : 'hover:bg-muted border border-transparent hover:border-border active:scale-95'
                                    }`}
                            >
                                {item.char}
                                <div
                                    ...
                                    className={`absolute top-1 right-1 p-1 rounded-full hover:bg-muted transition-opacity ${favorites.includes(item.id) ? 'opacity-100 text-yellow-500' : 'opacity-0 group-hover:opacity-100 text-muted-foreground'}`}
                                >
                        ...
                        {filteredItems.length === 0 && (
                            <div className="col-span-full h-40 flex flex-col items-center justify-center text-muted-foreground italic gap-2">
                ...
                {/* Composer */}
                <div className="bg-card p-3 rounded-xl border border-border shrink-0 flex gap-2">
                    <input
                        type="text"
                        value={composerText}
                        onChange={(e) => setComposerText(e.target.value)}
                        placeholder="Click an emoji to add..."
                        className="flex-1 bg-background border border-border rounded-lg px-3 py-2 text-lg text-foreground focus:outline-none focus:border-accent"
                    />
                    <button
                        onClick={() => copyText(composerText, 'Composer')}
                        className="px-4 py-2 bg-accent text-accent-foreground font-bold rounded-lg hover:opacity-90 transition-colors shadow-lg active:scale-95"
                    >
                        Copy
                    </button>
                    <button
                        onClick={() => setComposerText('')}
                        className="px-3 py-2 bg-secondary text-muted-foreground rounded-lg hover:text-destructive hover:bg-destructive/10 transition-colors"
                    >
                        {Icons.Trash}
                    </button>
                </div>
            ...
            <div className={`
          bg-card border-border flex flex-col shrink-0 overflow-hidden transition-all duration-200
      ...
      `}>
                {selectedEmoji ? (
                    <div className="flex flex-col h-full relative">
                        ...
                        <button
                            onClick={() => setSelectedEmoji(null)}
                            className="absolute top-2 right-2 p-2 bg-background/20 hover:bg-destructive rounded-full text-muted-foreground hover:text-destructive-foreground transition-colors z-10"
                            title="Close Details"
                        >
                            {Icons.Close}
                        </button>
                        ...
                        {/* Header Preview */}
                        <div className="h-48 bg-muted/30 flex flex-col items-center justify-center relative border-b border-border shrink-0">
                            ...
                            <div className="mt-4 text-center px-4">
                                <h3 className="text-lg font-bold text-foreground leading-tight">{selectedEmoji.name}</h3>
                                <span className="text-xs text-muted-foreground uppercase font-mono">{selectedEmoji.category}</span>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <button
                                    ...
                                    className="py-2.5 bg-accent text-accent-foreground rounded-lg font-bold hover:opacity-90 transition-colors shadow-lg active:scale-95 flex items-center justify-center gap-2"
                                >
                                    {Icons.Copy} Copy
                                </button>
                                <button
                                    onClick={() => { addToComposer(selectedEmoji.char); }}
                                    className="py-2.5 bg-blue-500/10 text-blue-500 border border-blue-500/50 rounded-lg font-bold hover:bg-blue-500 hover:text-white transition-all active:scale-95"
                                >
                                    + Composer
                                </button>
                            </div>
                            ...
                            <div className="space-y-3">
                                <h4 className="text-xs font-bold text-accent uppercase border-b border-border pb-1">Technical Codes</h4>
                            ...
                            {selectedEmoji.keywords.length > 0 && (
                                <div className="space-y-2">
                                    <h4 className="text-xs font-bold text-accent uppercase border-b border-border pb-1">Tags</h4>
                                    <div className="flex flex-wrap gap-1">
                                        {selectedEmoji.keywords.map(k => (
                                            <span key={k} className="px-2 py-1 bg-muted rounded text-[10px] text-muted-foreground">#{k}</span>
                                        ))}
                                    </div>
                                </div>
                            )}
                    ...
                    <div className="hidden lg:flex flex-1 flex-col items-center justify-center text-muted-foreground p-6 text-center">
                        ...
                        <p className="text-xs text-muted-foreground/60 mt-2">Single click adds to composer.</p>
                    </div>
                )}
            </div>
            ...
            {/* INFO / SETTINGS MODAL */}
            {showInfoModal && (
                <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setShowInfoModal(false)}>
                    <div className="bg-card border border-border rounded-xl shadow-2xl max-w-md w-full p-0 overflow-hidden animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                            ...
                            {/* Library Stats */}
                            <div className="space-y-3">
                                <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Library Statistics</h4>
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="bg-background p-3 rounded-lg border border-border">
                                        <div className="text-2xl font-bold text-foreground">{allData.length}</div>
                                        <div className="text-[10px] text-muted-foreground uppercase">Total Items</div>
                                    </div>
                                    <div className="bg-background p-3 rounded-lg border border-border">
                                        <div className="text-2xl font-bold text-yellow-500">{favorites.length}</div>
                                        <div className="text-[10px] text-muted-foreground uppercase">Favorites</div>
                                    </div>
                                </div>

                                <div className="bg-background rounded-lg border border-border p-3">
                                    <div className="text-[10px] text-muted-foreground uppercase mb-2">Category Breakdown</div>
                                    <div className="flex flex-wrap gap-2">
                                        {Object.entries(libraryStats).map(([cat, count]) => (
                                            <span key={cat} className="px-2 py-1 bg-muted rounded text-[10px] text-muted-foreground border border-border">
                                                {cat}: <span className="text-foreground font-bold">{count}</span>
                                            </span>
                                        ))}
                                    </div>
                                </div>
                            ...
                            {/* Controls Guide */}
                            <div className="space-y-3">
                                <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Usage Guide</h4>
                                <div className="bg-background rounded-lg p-4 border border-border space-y-4">
                                    <div className="flex items-start gap-3">
                                        <div className="p-2 bg-muted rounded text-xl">👆</div>
                                        <div>
                                            <div className="text-sm font-bold text-foreground">Single Tap / Click</div>
                                            <p className="text-xs text-muted-foreground">Instantly copies the emoji to clipboard and adds it to the bottom composer.</p>
                                        </div>
                                    </div>
                                    <div className="flex items-start gap-3">
                                        <div className="p-2 bg-muted rounded text-xl">🕵️</div>
                                        <div>
                                            <div className="text-sm font-bold text-foreground">Long Press / Double Click</div>
                                            <p className="text-xs text-muted-foreground">Opens the <strong>Inspector</strong> to view technical details (Unicode, HTML Entity) and tags.</p>
                                        </div>
                                    </div>
                                    <div className="flex items-start gap-3">
                                        <div className="p-2 bg-muted rounded text-xl">🔍</div>
                                        <div>
                                            <div className="text-sm font-bold text-foreground">Smart Search</div>
                                            <p className="text-xs text-muted-foreground">Search by name or keyword (e.g. "happy", "blue", "food").</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="p-4 border-t border-border bg-muted/30">
                            <button
                                ...
                                className="w-full py-2 bg-accent text-accent-foreground font-bold rounded-lg hover:opacity-90 transition-colors"
                            >
                                Close
                            </button>
                        </div>
            ...
            {/* Notification Toast */}
            {notification && (
                <div className="fixed bottom-4 right-4 z-[60] bg-green-500 text-white px-4 py-2 rounded-lg shadow-xl text-sm font-bold text-center animate-in fade-in slide-in-from-bottom-2">
                    {notification}
                </div>
            )}
...
const CodeRow = ({ label, code, onCopy }: { label: string, code: string, onCopy: (c: string) => void }) => (
    <div className="flex items-center justify-between bg-muted/50 rounded p-2 group">
        <div className="flex flex-col">
            <span className="text-[10px] text-muted-foreground font-bold uppercase">{label}</span>
            <span className="font-mono text-xs text-foreground select-all">{code}</span>
        </div>
        <button
            onClick={() => onCopy(code)}
            className="p-1.5 text-muted-foreground hover:text-foreground hover:bg-muted rounded transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100"
            title="Copy"
        >
            {Icons.Copy}
        </button>
