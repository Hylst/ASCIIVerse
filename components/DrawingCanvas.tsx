...
const DrawingCanvas: React.FC = () => {
    ...
    return (
        <div className="flex flex-col h-full gap-4 min-h-0" onPointerUp={handlePointerUp} onPointerLeave={handlePointerUp}>
            ...
            {/* Toolbar */}
            <div className="bg-card p-2 rounded-xl border border-border flex flex-wrap items-center gap-4 shrink-0 shadow-sm">
                ...
                <div className="flex gap-1 p-1 bg-muted/20 rounded-lg border border-border">
                    ...
                    <div className="w-px bg-border mx-1 self-center h-4" />
                ...
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1 bg-muted/20 p-1 rounded-lg border border-border">
                        <button onClick={() => setSymmetry(symmetry === 'none' ? 'x' : symmetry === 'x' ? 'y' : 'none')} className={`px-2 py-1 text-xs font-bold rounded ${symmetry !== 'none' ? 'bg-accent text-accent-foreground' : 'text-muted-foreground'}`}>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <input type="number" value={width} onChange={e => handleResize(parseInt(e.target.value), height)} className="w-12 bg-background border border-border rounded text-xs p-1 text-center text-foreground" />
                        <span className="text-muted-foreground/60">x</span>
                        <input type="number" value={height} onChange={e => handleResize(width, parseInt(e.target.value))} className="w-12 bg-background border border-border rounded text-xs p-1 text-center text-foreground" />
                    </div>

                    <div className="flex items-center gap-2">
                        <label className="text-[10px] uppercase font-bold text-muted-foreground hidden sm:inline">Zoom</label>
                        <input type="range" min="0.5" max="2" step="0.1" value={zoom} onChange={e => setZoom(parseFloat(e.target.value))} className="w-20 accent-accent h-1 bg-muted rounded appearance-none" />
                    </div>
                </div>
                ...
                <div className="flex gap-2">
                    <button onClick={undo} disabled={historyIndex <= 0} className="p-2 text-muted-foreground hover:text-foreground disabled:opacity-30"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 14 4 9l5-5" /><path d="M4 9h10.5a5.5 5.5 0 0 1 5.5 5.5v0a5.5 5.5 0 0 1-5.5 5.5H11" /></svg></button>
                    ...
                    <div className="w-px bg-border h-6 self-center mx-1"></div>
                    <button onClick={() => resetGrid(width, height)} className="p-2 text-destructive hover:bg-muted rounded" title="Clear"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 6h18" /><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6" /><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2" /></svg></button>
                    <button onClick={saveProject} className="p-2 text-muted-foreground hover:bg-muted rounded" title="Save"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z" /><polyline points="17 21 17 13 7 13 7 21" /><polyline points="7 3 7 8 15 8" /></svg></button>
                    <button onClick={copyToClipboard} className="bg-accent text-accent-foreground px-3 py-1 rounded text-xs font-bold hover:opacity-90 flex items-center gap-1">
                        {notification || 'Copy'}
                    </button>
                </div>
            </div>
            ...
            <div className="flex-1 flex gap-4 min-h-0 overflow-hidden">
                ...
                {/* Canvas Area */}
                <div className="flex-1 bg-background/50 border border-border rounded-xl overflow-auto relative custom-scrollbar flex items-center justify-center">
                    <div
                        className="relative bg-card shadow-2xl transition-transform duration-75 ease-out origin-center"
                        style={{
                            width: width * 24,
                            height: height * 24,
                            ...
                            {displayGrid.map((row, r) => (
                                row.map((cell, c) => (
                                    <div
                                        ...
                                        className={`
                                    flex items-center justify-center text-accent select-none
                                    ${showGridLines ? 'border-[0.5px] border-border/30' : ''}
                                    hover:bg-muted/50
                                `}
                ...
                {/* Sidebar Palette */}
                <div className="w-16 md:w-20 bg-card rounded-xl border border-border flex flex-col overflow-hidden shrink-0">
                    <div className="p-2 border-b border-border text-center">
                        <span className="text-[10px] font-bold text-muted-foreground uppercase">Chars</span>
                    </div>
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-2">
                        {DRAWING_CHARS.map((char, i) => (
                            <button
                                ...
                                className={`w-full aspect-square flex items-center justify-center rounded text-lg font-mono border transition-all ${selectedChar === char
                        ...
                        <div className="pt-2 border-t border-border/50">
                            <input
                                maxLength={1}
                                placeholder="+"
                                className="w-full h-10 bg-background border border-border rounded text-center text-foreground focus:border-accent focus:outline-none"
                    ...
                    <div className="p-2 border-t border-border">
                        <button onClick={() => setShowGridLines(!showGridLines)} className="w-full py-1 text-[10px] text-muted-foreground hover:text-foreground bg-background rounded">
...
const ToolBtn = ({ active, onClick, icon, title }: { active: boolean, onClick: () => void, icon: React.ReactNode, title: string }) => (
    <button
        onClick={onClick}
        title={title}
        className={`p-2 rounded-md transition-colors ${active ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-muted'}`}
    >
