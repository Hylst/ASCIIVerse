    ...
    <div className="flex flex-col h-full gap-4 min-h-0">
      <div className="flex flex-col lg:flex-row gap-4 h-full min-h-0">
        ...
        <div className="w-full lg:w-96 flex flex-col gap-3 min-h-0 h-1/2 lg:h-full shrink-0">
          <div className="bg-card p-3 rounded-xl border border-border flex flex-col gap-3 h-full overflow-hidden">
            ...
            <div className="shrink-0 space-y-3">
              <div className="flex justify-between items-center">
                ...
                <button
                  ...
                  className="text-xs bg-secondary hover:bg-muted text-muted-foreground px-2 py-1 rounded flex items-center gap-1 transition-colors"
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm focus:border-accent focus:outline-none pl-9"
                />
                <svg className="absolute left-3 top-2.5 text-muted-foreground" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8" /><line x1="21" x2="16.65" y1="21" y2="16.65" /></svg>
              </div>
            </div>
            ...
            <div className="flex gap-2 overflow-x-auto custom-scrollbar shrink-0 pb-1">
              {categories.map(cat => (
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-xs font-bold whitespace-nowrap border transition-all ${selectedCategory === cat
              ...
              {filteredPatterns.length === 0 ? (
                ...
                filteredPatterns.map(sep => (
                  <button
                    ...
                    className={`w-full text-left p-2 rounded-lg text-sm border transition-all group ${selectedId === sep.id
                  ...
                  >
                    ...
                    <div className={`font-mono text-xs truncate opacity-80 ${selectedId === sep.id ? 'text-foreground' : 'text-muted-foreground'}`}>
                      {getPreview(sep)}
                    </div>
                  </button>
                ))
        ...
        <div className="flex-1 flex flex-col gap-4 min-h-0 h-1/2 lg:h-full">
          ...
          {/* Preview Card */}
          <div className="flex-1 bg-background rounded-xl border border-border flex flex-col relative group overflow-hidden shadow-2xl min-h-[200px]">
            {/* Header Gradient */}
            ...
            <div className="absolute top-4 right-4 z-10">
              <button
                ...
                className={`px-4 py-2 rounded-lg font-bold text-sm flex items-center gap-2 transition-all transform active:scale-95 shadow-lg ${copied ? 'bg-green-500 text-white' : 'bg-accent text-accent-foreground hover:opacity-90'
                  }`}
              >
                {copied ? 'Copied!' : 'Copy'}
              </button>
            ...
            {/* Main Preview */}
            <div className="flex-1 overflow-auto custom-scrollbar flex items-center justify-center p-8 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-muted/50 to-background">
              <div className="text-center w-full">
                <div className="font-mono text-accent text-lg md:text-2xl lg:text-3xl break-all leading-normal drop-shadow-[0_0_15px_rgba(var(--accent-rgb),0.2)] whitespace-pre-wrap">
                  {output}
                </div>
              </div>
            </div>
          ...
          {/* Config Panel */}
          <div className="bg-card p-4 rounded-xl border border-border shrink-0 space-y-4">
            <div className="flex items-center justify-between border-b border-border pb-2">
              ...
              <span className="text-xs text-muted-foreground">{selectedPattern.name}</span>
            </div>

            {/* Pattern Parts Editing */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className="space-y-1">
                <label className="text-[10px] text-muted-foreground font-bold uppercase">Left Cap</label>
                <input
                  type="text"
                  value={customLeft}
                  onChange={(e) => setCustomLeft(e.target.value)}
                  className="w-full bg-background border border-border rounded p-2 text-sm text-center text-foreground focus:border-accent focus:outline-none font-mono"
                />
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                <label className="text-[10px] text-muted-foreground font-bold uppercase">Right Cap</label>
                <input
                  type="text"
                  value={customRight}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
              ...
              <div className={`space-y-2 transition-opacity ${!selectedPattern.repeat ? 'opacity-50 pointer-events-none' : ''}`}>
                <div className="flex justify-between">
                  <label className="text-xs text-muted-foreground font-bold">Repetitions</label>
                  <span className="text-xs text-accent font-mono">{length}</span>
                </div>
                <input
                  ...
                  className="w-full accent-accent h-1 bg-muted rounded-lg appearance-none cursor-pointer disabled:cursor-not-allowed"
                />
              </div>
              ...
              <div className="flex items-center gap-4 justify-end">
                <label className={`flex items-center gap-2 cursor-pointer ${!selectedPattern.repeat ? 'opacity-50 pointer-events-none' : ''}`}>
                  <input
                    ...
                    disabled={!selectedPattern.repeat}
                    className="w-4 h-4 text-accent rounded bg-background border-border focus:ring-offset-0 focus:ring-accent"
                  />
                  <span className="text-xs font-bold text-muted-foreground">Add Spacing</span>
                </label>
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    className="w-4 h-4 text-accent rounded bg-background border-border focus:ring-offset-0 focus:ring-accent"
                  />
                  <span className="text-xs font-bold text-muted-foreground">Mirror / Flip</span>
                </label>
              </div>
            </div>
          </div>
