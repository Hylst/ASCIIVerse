  ...
  return (
    <div className="flex flex-col h-full gap-6">
       <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-full min-h-0">
           ...
           <div className="flex flex-col gap-4 h-full min-h-0 overflow-hidden">
                ...
                <div className="bg-terminal-paper p-4 rounded-xl border border-slate-700 shrink-0">
                    ...
                    <textarea
                        ...
                        className="w-full h-32 bg-slate-900 border border-slate-700 rounded-lg p-3 font-mono text-xs text-terminal-text focus:outline-none focus:border-terminal-accent resize-none custom-scrollbar whitespace-nowrap"
                        placeholder="Paste CSV, Excel data, or JSON here..."
                    />
                    {error && <p className="text-red-400 text-xs mt-2 flex items-center gap-1"><span className="font-bold">Error:</span> {error}</p>}
                </div>

                {/* 2. Column Controls (Visual) */}
                {parsedData.headers.length > 0 && (
                    <div className="bg-terminal-paper p-3 rounded-xl border border-slate-700 shrink-0">
                        <h4 className="text-[10px] font-bold text-slate-500 uppercase mb-2">Column Alignment</h4>
                        <div className="flex gap-2 overflow-x-auto custom-scrollbar pb-1">
                            {parsedData.headers.map((h, i) => (
                                <button 
                                    key={i} 
                                    onClick={() => toggleColAlign(i)}
                                    className="flex-shrink-0 px-3 py-1.5 bg-slate-800 border border-slate-700 rounded-md text-xs font-mono text-slate-300 hover:border-terminal-accent transition-colors flex items-center gap-2 group"
                                    title="Click to toggle alignment"
                                >
                                    <span>{h}</span>
                                    <span className={`text-[10px] font-bold ${colConfigs[i]?.alignment !== 'left' ? 'text-terminal-accent' : 'text-slate-600'}`}>
                ...
                {/* 3. Global Settings */}
                <div className="bg-terminal-paper p-4 rounded-xl border border-slate-700 flex-1 overflow-y-auto custom-scrollbar">
                    <h3 className="text-terminal-accent font-bold mb-4 text-sm uppercase tracking-wider">Style Settings</h3>
                    <div className="grid grid-cols-2 gap-x-4 gap-y-6">
                        <div className="space-y-1">
                            <label className="text-xs text-terminal-muted font-bold">Border Style</label>
                            <select 
                                value={borderStyle}
                                onChange={(e) => setBorderStyle(e.target.value as BorderStyle)}
                                className="w-full bg-slate-900 border border-slate-700 rounded p-2 text-sm focus:outline-none focus:border-terminal-accent"
                            >
                        ...
                        <div className="space-y-1">
                            <label className="text-xs text-terminal-muted font-bold">Default Alignment</label>
                            <select 
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between">
                                <label className="text-xs text-terminal-muted">Horizontal Padding</label>
                                <span className="text-xs text-terminal-accent">{padding}</span>
                            </div>
                            <input 
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between">
                                <label className="text-xs text-terminal-muted">Vertical Padding</label>
                                <span className="text-xs text-terminal-accent">{verticalPadding}</span>
                            </div>
                            <input 
                        ...
                        <div className="col-span-2">
                             <label className="flex items-center gap-3 cursor-pointer bg-slate-900/50 p-2 rounded-lg border border-slate-800 hover:border-slate-600 transition-colors">
                                <input 
                                    type="checkbox" 
                                    checked={mergeCells} 
                                    onChange={(e) => setMergeCells(e.target.checked)}
                                    className="w-4 h-4 text-terminal-accent rounded focus:ring-terminal-accent bg-slate-800 border-slate-600"
                                />
                                <div className="flex flex-col">
                                    <span className="text-xs font-bold text-slate-300">Smart Merge</span>
                                    <span className="text-[9px] text-slate-500">Merges identical adjacent cells horizontally</span>
                                </div>
                            </label>
                        </div>
                    </div>
           ...
           {/* --- RIGHT COLUMN: Output --- */}
           <div className="flex flex-col h-full bg-black rounded-xl border border-slate-800 overflow-hidden relative group">
                <div className="absolute top-4 right-4 z-10 flex gap-2">
                    <span className="px-2 py-1 rounded bg-slate-900/80 text-[10px] text-slate-500 border border-slate-800">
                        {outputTable.length} chars
                    </span>
                    <button 
                        onClick={copyToClipboard}
                        className="bg-terminal-accent text-terminal-bg px-4 py-1.5 rounded-lg font-bold text-xs hover:bg-emerald-400 flex items-center gap-2 shadow-lg active:scale-95 transition-all"
                    >
                        Copy
                    </button>
                </div>
                ...
                <div className="h-8 bg-slate-900 border-b border-slate-800 flex items-center px-4">
                    ...
                    <div className="mx-auto text-[10px] text-slate-600 font-mono uppercase tracking-widest">Table Preview</div>
                </div>

                <div className="p-6 overflow-auto custom-scrollbar flex items-start justify-center min-h-0 flex-1">
                    <pre className="font-mono text-emerald-400 text-xs md:text-sm whitespace-pre leading-relaxed drop-shadow-[0_0_8px_rgba(16,185,129,0.15)]">
                        {outputTable}
                    </pre>
                </div>
           </div>
